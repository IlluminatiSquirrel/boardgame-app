...
export default function BoardgamesList() {
  ...
  useEffect(() => {
    fetch('https://www.boardgamegeek.com/xmlapi2/hot?type=boardgames')
      ...
      .then(
        (hotBoardgamesResult) => {
          const idList: ReadonlyArray<string> = xml2js(hotBoardgamesResult).elements[0].elements.map((element) => element.attributes.id);

          fetch(`http://127.0.0.1:8080/https://www.boardgamegeek.com/xmlapi2/thing?stats=1&id=${idList}`)
            .then((res) => res.text())
            .then(
              (boardgameDetailsResult) => {
    ...
    <div>
      {items.map((value) => (
        <div key={value.id} className="boardgame-container">
          <img className="boardgame-image" alt="" src={value.thumbnail} />
